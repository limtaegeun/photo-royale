...
- 라이브러리 추가 전 반드시 사용자에게 확인 → 현재 런타임 의존성은 vue/pinia/vue-router 3개뿐이며 이 상태를 유지하는 것이 기본값

## 알아둘 것

- `src/stores/counter.ts`와 `src/__tests__/App.spec.ts`는 create-vue 스캐폴드 데모다. 첫 실제 기능이 들어갈 때 삭제한다.
